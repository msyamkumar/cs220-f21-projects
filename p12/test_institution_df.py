...
REQUIRED_FILES = ['institution_expected.html']
...
questions = [
    # stage 1
    Question(number=21, weight=0, format=HTML_FORMAT),
]
...
expected_files = ['institution_expected.html']
...
def rerun_notebook(orig_notebook):
    new_notebook = 'cs-220-test_instituition.ipynb'
...
def check_cell_html(qnum, cell):
    ...
    try:
        with open('institution_expected.html', encoding='utf-8') as f:
            expected_cells = parse_df_html_table(f.read(), qnum)
    except Exception as e:
        print("ERROR!  Could not find table in institution_expected.html")
        raise e
